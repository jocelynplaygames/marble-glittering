...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if user is already subscribed to marble
    ...
    if (subscriptionExists) {
      return new Response("You've already subscribed to this marble", {
        status: 400,
      });
    }
    ...
    return new Response(
      "Could not subscribe to marble at this time. Please try again later",
      { status: 500 },
    );
  }
}
