...
export function JoinLeaveToggle({
  ...
  const { mutate: subscribe, isLoading: isSubLoading } = useMutation({
    mutationFn: async () => {
      const payload: SubscribeToMarblePayload = {
        marbleId,
      };

      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      const { data } = await axios.post("/api/marble/subscribe", payload);
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Subscribed!",
        description: `You are now subscribed to r/${marbleName}`,
      });
    },
  });
  ...
  const { mutate: unsubscribe, isLoading: isUnsubLoading } = useMutation({
    mutationFn: async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      const { data } = await axios.post("/api/marble/unsubscribe", payload);
      return data as string;
    },
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Unsubscribed!",
        description: `You are now unsubscribed from/${marbleName}`,
      });
    },
  });
